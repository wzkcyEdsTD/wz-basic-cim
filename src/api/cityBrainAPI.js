...
instance.interceptors.request.use(
  async config => {
    if (!config.url.match(/\/oauth/g)) {
      ...
      config.headers.post['Access-Control-Allow-Headers'] = 'Content-Type,XFILENAME,XFILECATEGORY,XFILESIZE';
      config.headers.post['Access-Control-Allow-Methods'] = 'POST, GET, OPTIONS, DELETE';
    }
...
export const getExtraVideo = (data) => {
  return getAxios("http://10.36.198.161:3000/forward/getExtraVideo", data);
}
/**
